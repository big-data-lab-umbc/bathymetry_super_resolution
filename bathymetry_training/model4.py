...
for epoch in range(epoch, n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        # Total loss
        loss_G = loss_content + 0.5 * weighted_content
        ...
        if (batches_done+1) % sample_interval == 0:
            ...
            save_image(img_grid, "images/baseline2_weight/%d.png" % batches_done, normalize=False) #save whole table: lr, gen_hr and hr

            # print("batch_done:%d\n" %batches_done)
  
    if checkpoint_interval != -1 and (epoch+1) % checkpoint_interval == 0:
        # Save model checkpoints
        torch.save(generator.state_dict(), "saved_models/baseline2_weight/generator_%d.pth" % epoch)


from matplotlib import pyplot
np.save('Loss/baseline2_weight.npy',Gloss)

pyplot.figure(figsize=(16,9))
pyplot.plot(Gloss, label='Generator')
pyplot.legend()
pyplot.savefig('baseline2_weight.png') 
pyplot.show()
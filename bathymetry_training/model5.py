...
if epoch != 0:
    ...
    generator.load_state_dict(torch.load("/home/ubuntu/super_resolution/PyTorch-GAN/implementations/srgan/model/baseline1.pth"))
...
dataloader = DataLoader(
    ImageDatasetPretrainMask("../../data2/train/", hr_shape=hr_shape),
...
for epoch in range(epoch, n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        sys.stdout.write(
            "[Epoch %d/%d] [Batch %d/%d] [G loss: %f][Weight: %f]\n"
            % (epoch, n_epochs, i+1, len(dataloader), loss_G.item(),weighted_content.item())
        )
        ...
        if (batches_done+1) % sample_interval == 0:
            ...
            with rasterio.Env():
                ...
                with rasterio.open("images/approach9_vgg19_400/gen_{}_{}.tif".format(batches_done,hr_name[:-10]), 'w', **profile) as dst:
            ...
            #             transform = t 
            #         )
            #     with rasterio.open("images/baseline2_weight/gen_{}_{}.tif".format(batches_done,hr_name[:-10]), 'w', **profile) as dst:
            ...
            save_image(img_grid, "images/approach9_vgg19_400/%d.png" % batches_done, normalize=False) #save whole table: lr, gen_hr and hr

            # print("batch_done:%d\n" %batches_done)
  
    if checkpoint_interval != -1 and (epoch+1) % checkpoint_interval == 0:
        # Save model checkpoints
        torch.save(generator.state_dict(), "saved_models/approach9_vgg19_400/generator_%d.pth" % epoch)


from matplotlib import pyplot
np.save('Loss/approach9_vgg19_400.npy',Gloss)

pyplot.figure(figsize=(16,9))
pyplot.plot(Gloss, label='Generator')
pyplot.legend()
pyplot.savefig('approach9_vgg19_400.png') 
pyplot.show()
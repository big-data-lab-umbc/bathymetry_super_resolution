...
if opt.epoch != 0:
    ...
    generator.load_state_dict(torch.load("/home/ubuntu/PyTorch-GAN/implementations/srgan/model/generator_44.pth"))
...
dataloader = DataLoader(
    # ImageDatasetPrePNG("../../data/img_celebrate_part/", hr_shape=hr_shape),
    ImageDatasetPretrainTiff("../../data/", hr_shape=hr_shape),
        ...
        if (batches_done+1) % opt.sample_interval == 0:
            ...
            with rasterio.Env():
                with rasterio.open(input_path[0]) as src: 
                    ...
                    profile.update(
                        width = opt.hr_height, 
                        height = opt.hr_height,
                        dtype = rasterio.float32,
                        transform = t 
                    )
                with rasterio.open("images/pretrained_to_tif/gen_{}_{}.tif".format(hr_name,batches_done), 'w', **profile) as dst:
                    dst.write(gen_hr_single[0][0], indexes = 1)

            # Save PNG image grid with upsampled inputs and SRGAN outputs
            imgs_lr = nn.functional.interpolate(imgs_lr, scale_factor=4)
            gen_hr = make_grid(gen_hr, nrow=1, normalize=True)
            imgs_lr = make_grid(imgs_lr, nrow=1, normalize=True)
            imgs_hr = make_grid(imgs_hr, nrow=1,normalize=True)
            img_grid = torch.cat((imgs_lr, gen_hr, imgs_hr), -1)
            save_image(img_grid, "images/pretrained_to_tif/%d.png" % batches_done, normalize=False)

...
# Parameters
exp_name = "train_on_interpolation_edge_3" # Experiment name used to save loss plot
...
epoch = 1     # Set 0 if no pretrained model used; Set 1 if use a pretrained model
n_epochs=200
batch_size=4
lr=0.0002
b1=0.5
...
for epoch in range(epoch, n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        if mask_edge.sum() != 0:
            loss_edge_balance = loss_edge / mask_edge.sum() * (hr_height*hr_height*batch_size*channels)
            loss_G = 1.2* loss_content + 0.08 * loss_edge_balance
        else:
            loss_G = 1.2* loss_content
        ...
        sys.stdout.write(
            ...
            % (epoch+1, n_epochs, i+1, len(dataloader), 0.6*loss_G.item(), 0.4*loss_edge_balance.item())
        )
        ...
        if (batches_done+1) % sample_interval == 0:
            ...
            #         )
            #         #print(profile)
            #     with rasterio.open("images/noscale13/{}_{}.tif".format(hr_name,batches_done), 'w', **profile) as dst:
            ...
            # save_image(img_grid, "images/noscale13/%d.png" % batches_done, normalize=False) 
    ...
    if checkpoint_interval != -1 and (epoch+1) % checkpoint_interval == 0:
        ...
        torch.save(generator.state_dict(), "saved_models/noscale13/generator_%d.pth" % epoch)

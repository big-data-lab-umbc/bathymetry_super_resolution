...
# Parameters
exp_name = "train_on_interpolation_edge_1" # Experiment name used to save loss plot
...
for epoch in range(epoch, n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        if mask_edge.sum() != 0:
            ...
            loss_G = loss_content + 0.1 * loss_edge_balance
        else:
            loss_G = loss_content
        ...
        sys.stdout.write(
            ...
            % (epoch+1, n_epochs, i+1, len(dataloader), loss_G.item(), loss_edge_balance.item())
        )
    ...
    if checkpoint_interval != -1 and (epoch+1) % checkpoint_interval == 0:
        ...
        torch.save(generator.state_dict(), "saved_models/noscale11/generator_%d.pth" % epoch)

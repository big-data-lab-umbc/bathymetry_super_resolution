...
# Parameters
exp_name = "train_on_interpolation_exp2" # Experiment name used to save loss plot
...
for epoch in range(epoch, n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        loss_G = loss_content.mean() + 0.5 * (water_content.mean() + weighted_content.mean())
        ...
        sys.stdout.write(
            ...
            % (epoch+1, n_epochs, i+1, len(dataloader), loss_G.item(), loss_content.mean().item(), water_content.mean().item())
        )
        ...
        if (batches_done+1) % sample_interval == 0:
            ...
            save_image(img_grid, "images_exp2/%d.png" % batches_done, normalize=False) 
    ...
    if checkpoint_interval != -1 and (epoch+1) % checkpoint_interval == 0:
        ...
        torch.save(generator.state_dict(), "saved_models_exp2/generator_%d.pth" % epoch)

...
# Parameters
exp_name = "train_on_interpolation_exp5" # Experiment name used to save loss plot
...
for epoch in range(epoch, n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        if mask_edge.sum() != 0:
            ...
            loss_G = loss_content.mean() + 0.1 * loss_edge_balance.mean()
        else:
        ...
        sys.stdout.write(
            ...
            % (epoch+1, n_epochs, i+1, len(dataloader), loss_G.item(), loss_edge_balance.mean().item())
        )
        ...
        if (batches_done+1) % sample_interval == 0:
            ...
            #         )
            #         #print(profile)
            #     with rasterio.open("images/noscale11/{}_{}.tif".format(hr_name,batches_done), 'w', **profile) as dst:
            ...
            # save_image(img_grid, "images/noscale11/%d.png" % batches_done, normalize=False) 
    ...
    if checkpoint_interval != -1 and (epoch+1) % checkpoint_interval == 0:
        ...
        torch.save(generator.state_dict(), "saved_models_exp4/generator_%d.pth" % epoch)

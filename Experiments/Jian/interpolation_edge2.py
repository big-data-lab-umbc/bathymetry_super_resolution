...
# Parameters
exp_name = "train_on_interpolation_exp6" # Experiment name used to save loss plot
...
for epoch in range(epoch, n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        if mask_edge.sum() != 0:
            ...
            loss_G = 1.2* loss_content.mean() + 0.08 * loss_edge_balance.mean()
        else:
            loss_G = 1.2* loss_content.mean()
        ...
        sys.stdout.write(
            ...
            % (epoch+1, n_epochs, i+1, len(dataloader), 0.6*loss_G.item(), 0.4*loss_edge_balance.mean().item())
        )
    ...
    if checkpoint_interval != -1 and (epoch+1) % checkpoint_interval == 0:
        ...
        torch.save(generator.state_dict(), "saved_models_exp5/generator_%d.pth" % epoch)

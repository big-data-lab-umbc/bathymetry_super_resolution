...
# Parameters
exp_name = "train_on_interpolation_exp3" # Experiment name used to save loss plot
...
n_cpu=8
channels=3
sample_interval=400 # How many intervals to save a training result each time
...
for epoch in range(epoch, n_epochs):
    for i, imgs in enumerate(dataloader):
        ...
        mask = Variable(imgs["mask_ocean"].type(Tensor))
        ...
        loss_G = weighted_content.mean() + loss_content.mean()
        ...
        sys.stdout.write(
            ...
            % (epoch+1, n_epochs, i+1, len(dataloader), loss_G.item(), weighted_content.item())
        )
        ...
        if (batches_done+1) % sample_interval == 0:
            ...
            if False:
                ...
                with rasterio.Env():
                    with rasterio.open(input_path[0]) as src: 
                        ...
                        #print(profile)
                    with rasterio.open("images/noscale6/{}_{}.tif".format(hr_name,batches_done), 'w', **profile) as dst:
            ...
            save_image(img_grid, "images_exp3/%d.png" % batches_done, normalize=False) 
    ...
    if checkpoint_interval != -1 and (epoch+1) % checkpoint_interval == 0:
        ...
        torch.save(generator.state_dict(), "saved_models_exp3/generator_%d.pth" % epoch)
